    ...
    <div class="td-game-wrapper" :class="{is_win: game.is_win, is_loss: (!game.is_win && game.final_score) }">
        <div class="game-data">
            ...
            <div class="final-score">
                ...
                <div v-else-if="(game.is_win===false && game.cancelled_or_postponed===true)" class="unplayed_info">PPD</div>
                <div v-else-if="(!game.cancelled_or_postponed && game.game_time)" class="unplayed_info">{{ game.game_time }}</div>
            </div>
        </div>
    </div>
</template>
